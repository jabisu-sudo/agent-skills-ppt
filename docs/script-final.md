...
三个月，**10 万+ skills**，主流 AI 编程平台基本都采纳了。

这速度说明什么？说明大家真的很需要这个东西。

而且 skills 不只是写代码用的。我自己装的就有查天气的、管 Apple Reminders 的、港股打新分析的、甚至还有帮我发 iMessage 的。它已经渗透到开发、生活、工作流的方方面面。

今天我们就来聊聊：它解决什么问题，怎么写好一个 skill，以及——安全问题有多严重。
...
用过 ChatGPT、Copilot、Claude 的都知道——**AI 不记事**。

你今天跟它说"我们项目用 pnpm，不要用 npm"，明天它又给你生成 `npm install`。你说"commit message 用 conventional commits 格式"，下次它又忘了。

这不是 AI 笨，是架构问题。每次对话都是新的 session，context 是空的。
...
**方案一：每次手动告诉它。**
"记住，我们用 pnpm，不用 npm。" 每次都说一遍。累，而且占 context。

**方案二：在 system prompt 里写一大堆规则。**
把所有你想让 AI 记住的东西都塞进去。问题是 context window 是有限的，你塞 1 万字的规则进去，留给真正干活的空间就少了。

**方案三：RAG（检索增强生成）。**
把知识库向量化，查询时检索相关内容注入。这个技术上可行，但对个人开发者来说太重了，而且检索质量不稳定。

**方案四：Fine-tuning。**
训练一个专属模型。成本高，更新慢，不现实。

这些方案要么太累，要么太贵，要么太重。

#### Skill 的两个核心价值

Agent Skills 的思路不一样。它的核心价值就两个：
...
这是技术上最精妙的地方。Skill 不是一股脑全部塞进 context 的。
...
所以 skills 解决的问题是：让 AI 记住该记的东西，但又不浪费宝贵的 context 空间。
...
很多 skill 带脚本。你装了一个看起来正常的 skill，但它的 scripts/ 目录里有：
...
跟 npm 投毒一样。攻击者发布一个有用的 skill，等大家装了，再更新一个恶意版本。或者名字取得像热门 skill（typosquatting），比如 `weather-skil`（少一个 l）。
...
- **Context 膨胀**：skill 越装越多，路由信息也会膨胀。虽然比全量加载好很多，但还是有上限。
- **Skill 冲突**：两个 skill 的 description 很像，AI 不知道选哪个。或者两个 skill 给出矛盾的指令。
- **动态信息**：Skill 是静态的，但很多知识是变化的。API 变了、最佳实践更新了，skill 怎么保持更新？
...
1. Agent Skills 用三层加载解决了"AI 失忆"和"context 不够用"的问题
2. 核心价值是**可复用**和**省 context**
3. 写 skill 最重要的是 description，它是路由信号
4. 安全问题很严重，三分之一有问题，要当心
5. 生态还在早期，很多东西还在演进
